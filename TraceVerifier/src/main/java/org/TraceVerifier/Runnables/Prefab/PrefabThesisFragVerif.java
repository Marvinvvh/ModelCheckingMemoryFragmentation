 ...
 * This runnable is specifically for comparing whether the fragmentation has the expected values. See Thesis-256KiB for the allocator comparisons.
 */
public class PrefabThesisFragVerif {
    public static void run(TracePipelineConfig pipelineConfig, Path modelLocation, Path traceLocation) throws IOException {
        ...
        // Shared vars traces
        Path comparisonTracePath = Path.of(traceLocation.toString(), "fragverif_trace");

        // Synth traces
        TraceGroup comparison = TraceImportUtil.getTraceGroupFromDir(comparisonTracePath, "glob:**/*");
