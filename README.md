# Dataset and Source Code MODEL CHECKING MEMORY FRAGMENTATION OF ALLOCATOR POLICIES USING TIMED AUTOMATA
...
4. Testing: Contains the fragverif and validation datasets. The fragverif dataset is used in verifying the points and fragmentation values in TraceAnalyzer, and the validation dataset contains the validation queries that have been executed and successfully passed.
5. TraceAnalyzer: Contains TraceAnalyzer, which performs preparing datasets for analysis, policy comparisons, simulation performance analysis, similarity comparisons between datasets, point and fragmentation validation with fragverif, and point scaling tests. It contains the source code and tests. The datasets are retrieved from the other folders. Not expanded upon, contains an additional README.
6. TraceVerifier: Contains TraceVerifier, which generates allocation traces, performs the simulations of the benchmark and validation, and creates the datasets for analysis. It contains the source code, and scripts. Not expanded upon, contains an additional README.
...
Both benchmark and testing directories contain *_raw folders. These contain data from TraceVerifier. In general, these directories contain an execution.log at the top (except validation in test, explained later), and a run_0 directory. The general layout in the run directory is in order:
...
The validation dataset holds all validation queries and models, created during the verification pipeline in TraceVerifier. The execution logs are not stored per type of validation for each merged model, e.g., 'run_0/merged-model_0_BFAO-1-size-multiple/BFAO/execution.log'.
